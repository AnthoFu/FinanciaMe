...
const moveDirectories = async (userInput) => {
  try {
    ...
    for (const dir of oldDirs) {
      ...
      if (fs.existsSync(oldDirPath)) {
        if (userInput === "y") {
          ...
          console.log(`➡️ /${dir} movido a /${exampleDir}/${dir}.`);
        } else {
          await fs.promises.rm(oldDirPath, { recursive: true, force: true });
          console.log(`❌ /${dir} eliminado.`);
        }
      } else {
        console.log(`➡️ /${dir} no existe, omitiendo.`);
      }
    }
    ...
    console.log("\n✅ Reseteo del proyecto completado. Próximos pasos:");
    console.log(
...
rl.question(
  ...
  (answer) => {
    ...
    if (userInput === "y" || userInput === "n") {
      ...
      console.log("❌ Entrada no válida. Por favor, introduce 'Y' o 'N'.");
      rl.close();
    }
  }
);
