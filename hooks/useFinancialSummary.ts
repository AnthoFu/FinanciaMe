...
export const useFinancialSummary = (
  ...
  const summary = useMemo(() => {
    ...
    const calculateForWallets = (filteredWallets: Wallet[]) => {
      const byCurrency = filteredWallets.reduce(
        (acc, wallet) => {
      ...
      const consolidatedBcv = filteredWallets.reduce((total, wallet) => {
      ...
      const consolidatedAverage = filteredWallets.reduce((total, wallet) => {
        if (wallet.currency === 'VES') {
    ...
    const regularWallets = wallets.filter((w) => !w.isSavings);
    const savingsWallets = wallets.filter((w) => w.isSavings);

    return {
      regular: calculateForWallets(regularWallets),
      savings: calculateForWallets(savingsWallets),
    };
